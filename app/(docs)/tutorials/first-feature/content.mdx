...
If the slash commands still feel unfamiliar, go back to [Core Session Commands](/tutorials/core-commands) before moving on.

## Advanced: Match the Starter Files

This first feature flow eventually becomes the shared starter baseline:

- [CLAUDE.md.template](/configuration) commits the verification-first workflow as the default repo contract.
- [test-forward](/tutorials/skills) turns the RED -> GREEN expectations into a reusable shared skill.
- [the starter hook stack](/tutorials/first-hook) blocks source edits before RED, formats edits, runs related tests, and gates completion.
