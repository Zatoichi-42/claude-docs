export const metadata = {
  title: "Project Setup",
  description:
    "Build the shared repo baseline after the daily workflow is familiar: choose a bootstrap path, tighten CLAUDE.md, add shared settings, then layer in skills and optional hooks.",
}

# Project Setup

This page is the capstone, not the starting point. Read it after [First-Time Setup](/getting-started), [Daily Workflow](/workflows/daily), [Using Skills](/tutorials/skills), and [Your First Hook](/tutorials/first-hook).

The goal here is to turn the workflow the team already understands into a durable repo baseline.

## Before you start

You are ready for this page if the team can already answer these questions:

- What does a good Claude Code session look like in this repo?
- Which verification commands matter?
- Which prompts or skills already work well enough to standardize?
- Which rules actually need enforcement instead of just documentation?

If those answers are still fuzzy, go back to the tutorials first.

## Choose your bootstrap path

Both paths are valid:

| Path | Best for | Command |
| --- | --- | --- |
| Fast baseline | Teams that want a quick `CLAUDE.md` starting point and will tighten it manually. | `/init` |
| Guided baseline | Teams that want a broader interactive walkthrough before they commit the shared setup. | `CLAUDE_CODE_NEW_INIT=1 claude` |

The important decision is not which command is more advanced. The important decision is whether the team understands the baseline it is about to standardize.

## Layer 1: Build the minimum baseline

Start with the smallest useful shared setup:

1. a tight `CLAUDE.md`
2. a shared `.claude/settings.json`
3. a short list of verification commands
4. clear separation between team-shared config and personal overrides

### A tight `CLAUDE.md`

Keep it short enough that the team will actually maintain it.

```markdown
# Project Working Rules

## Default workflow
- Explore before editing for multi-file or ambiguous work.
- For behavior changes: plan -> RED -> GREEN -> review.
- Keep diffs small and task-focused.

## Verification
- Targeted test: pnpm test -- <target>
- Lint: pnpm lint
- Typecheck: pnpm typecheck
- Full gate: pnpm lint && pnpm typecheck && pnpm test

## Repo notes
- Add the architectural facts Claude repeatedly needs in this codebase.
```

### Shared settings

Commit `.claude/settings.json` for the team. Keep personal changes in `.claude/settings.local.json`.

Start small:

- set the model and permissions you are comfortable with
- pre-approve only the commands the team uses constantly
- deny obvious secret paths up front
- skip heavy hook automation until it is clearly justified

## Layer 2: Add shared skills before heavy enforcement

Skills are usually the next thing worth standardizing because they capture a workflow pattern without blocking the session.

Good early candidates:

- a verification-first skill
- a repo-specific release flow
- a debugging checklist for a recurring failure mode

If the team cannot yet explain a skill in plain English, it is too early to commit it to the repo.

## Layer 3: Add hooks only where they remove real risk

Hooks are useful, but they are not the first thing most teams need.

Start with one of these:

- one **safety hook** that blocks something obviously risky
- one **feedback hook** that adds formatting or a related verification step

Do **not** start with a giant event matrix. If the team is still learning the workflow, too many hooks will feel like random resistance instead of helpful guardrails.

For the full event reference, use [Hooks & Events](/configuration/hooks).

## Layer 4: Review plugins, MCP, and agents intentionally

Open the interactive plugin manager first:

```text
/plugin
```

The right flow is:

1. inspect what is already installed
2. confirm what the repo actually needs
3. add only the missing plugins that solve a real problem

If the repo needs external tools through MCP, keep the first server personal while you are learning, then move it to project scope only when the team genuinely needs the same integration.

Do the same with custom agents. Add a reviewer or specialist only when the team already understands the narrow job it should own.

## Downloadable setup files

Use the individual downloads below if you only need one file, or grab the starter kit zip to get the complete `.claude/` structure in one shot.

<StarterKit />

<FileDownload
  filename="foundation-pack.zip"
  path="/downloads/foundation-pack.zip"
  label="Foundation pack"
  destination="CLAUDE.md + settings.json + .gitignore additions"
/>
<FileDownload
  filename="verification-pack.zip"
  path="/downloads/verification-pack.zip"
  label="Verification pack"
  destination="hooks + rules + test-forward skill"
/>
<FileDownload
  filename="ci-cd-pack.zip"
  path="/downloads/ci-cd-pack.zip"
  label="CI/CD pack"
  destination="settings + defer-ready automation hooks"
/>
<FileDownload
  filename="advanced-hooks-pack.zip"
  path="/downloads/advanced-hooks-pack.zip"
  label="Advanced hooks pack"
  destination="CLAUDE.md addendum + advanced settings + shell hook stack"
/>

<FileDownload
  filename="CLAUDE.md.template"
  path="files/CLAUDE.md.template"
  label="CLAUDE.md template"
  destination="CLAUDE.md"
/>
<FileDownload
  filename="CLAUDE.advanced-hooks.md"
  path="files/CLAUDE.advanced-hooks.md"
  label="CLAUDE.md addendum: advanced hooks"
  destination="merge into CLAUDE.md"
/>
<FileDownload
  filename="settings.json"
  path="files/settings.json"
  label="Shared settings.json"
  destination=".claude/settings.json"
/>
<FileDownload
  filename="settings.advanced-hooks.json"
  path="files/settings.advanced-hooks.json"
  label="Advanced hook settings"
  destination=".claude/settings.advanced-hooks.json"
/>
<FileDownload
  filename="block-dangerous.sh"
  path="files/hooks/block-dangerous.sh"
  label="Hook: block-dangerous.sh"
  destination=".claude/hooks/block-dangerous.sh"
/>
<FileDownload
  filename="require-test-first.sh"
  path="files/hooks/require-test-first.sh"
  label="Hook: require-test-first.sh"
  destination=".claude/hooks/require-test-first.sh"
/>
<FileDownload
  filename="auto-format.sh"
  path="files/hooks/auto-format.sh"
  label="Hook: auto-format.sh"
  destination=".claude/hooks/auto-format.sh"
/>
<FileDownload
  filename="run-related-tests.sh"
  path="files/hooks/run-related-tests.sh"
  label="Hook: run-related-tests.sh"
  destination=".claude/hooks/run-related-tests.sh"
/>
<FileDownload
  filename="defer-production-ops.sh"
  path="files/hooks/defer-production-ops.sh"
  label="Hook: defer-production-ops.sh"
  destination=".claude/hooks/defer-production-ops.sh"
/>
<FileDownload
  filename="hooklib.sh"
  path="files/hooks/hooklib.sh"
  label="Hook helper: hooklib.sh"
  destination=".claude/hooks/hooklib.sh"
/>
<FileDownload
  filename="session-start.sh"
  path="files/hooks/session-start.sh"
  label="Hook: session-start.sh"
  destination=".claude/hooks/session-start.sh"
/>
<FileDownload
  filename="pre-tool-use-guard.sh"
  path="files/hooks/pre-tool-use-guard.sh"
  label="Hook: pre-tool-use-guard.sh"
  destination=".claude/hooks/pre-tool-use-guard.sh"
/>
<FileDownload
  filename="post-tool-use-audit.sh"
  path="files/hooks/post-tool-use-audit.sh"
  label="Hook: post-tool-use-audit.sh"
  destination=".claude/hooks/post-tool-use-audit.sh"
/>
<FileDownload
  filename="stop-guard.sh"
  path="files/hooks/stop-guard.sh"
  label="Hook: stop-guard.sh"
  destination=".claude/hooks/stop-guard.sh"
/>
<FileDownload
  filename="frontend-testing.md"
  path="files/rules/frontend-testing.md"
  label="Scoped rule: frontend-testing.md"
  destination=".claude/rules/frontend-testing.md"
/>
<FileDownload
  filename="backend-testing.md"
  path="files/rules/backend-testing.md"
  label="Scoped rule: backend-testing.md"
  destination=".claude/rules/backend-testing.md"
/>
<FileDownload
  filename="SKILL.md"
  path="files/skills/test-forward/SKILL.md"
  label="Skill: test-forward"
  destination=".claude/skills/test-forward/SKILL.md"
/>
<FileDownload
  filename="code-reviewer.md"
  path="files/agents/code-reviewer.md"
  label="Custom agent: code-reviewer"
  destination=".claude/agents/code-reviewer.md"
/>
<FileDownload
  filename="gitignore.append"
  path="files/gitignore.append"
  label=".gitignore additions"
  destination=".gitignore"
/>

## What “done” looks like

You are finished with project setup when the repo has:

- a clear `CLAUDE.md`
- a shared settings baseline the team understands
- one or two shared skills or rules that reinforce the workflow
- optional hooks only where they add real value

If you are deciding how to roll that baseline across a team, continue with [Team Leader Setup](/configuration/team-leads).
